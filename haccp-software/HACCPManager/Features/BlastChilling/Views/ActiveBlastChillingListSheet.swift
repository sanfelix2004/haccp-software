...
struct ActiveBlastChillingListSheet: View {
    ...
    private func blastRow(_ snapshot: ActiveBlastSnapshot) -> some View {
        TimelineView(.periodic(from: .now, by: 1.0)) { timeline in
            let overRecommended = snapshot.isOverRecommended(at: timeline.date)
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top) {
                    ...
                    LiveProcessDurationText(
                        ...
                        color: overRecommended ? theme.colorWarning : Color.cyan
                    )
                }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: theme.spacing.cornerMedium, style: .continuous)
                    .stroke(
                        overRecommended ? theme.colorWarning.opacity(0.5) : theme.colorDivider,
                        lineWidth: 1
                    )
            )
        }
